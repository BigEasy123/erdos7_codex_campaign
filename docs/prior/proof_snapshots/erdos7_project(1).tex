...
second $675\times323323$ ``heavy-fibre'' necessary condition for the boundary shape.
\end{abstract}
...
\item \textbf{Heavy-set compatibility on the $675$ base.}  Build on
\eqref{eq:heavy-fibre}: the current pair-moment estimate treats the heavy sets for different
$d\mid R$ independently.  The next target is a cross-$d$ inequality exploiting the fact that the
corresponding cofactor residue classes must also cover the same points of the square-free group
$\Z/R\Z$.  A successful bound should control the weighted union of heavy fibres rather than its
separate one-$d$ marginals.
